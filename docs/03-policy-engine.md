...
| **滑点保护** ⚡ | DEX 交易的滑点控制 | 滑点不超过 1%（进阶实现） |
| **紧急暂停** ⚡ | 异常行为自动暂停 | 余额变动 > 20% 时暂停（进阶实现） |

> 💡 **白名单 vs 黑名单：什么时候用哪个？**
>
> 这两种策略看起来"反向"，但适用场景完全不同：
>
> | 维度 | 白名单（Whitelist） | 黑名单（Blacklist） |
> |------|---------------------|---------------------|
> | **默认行为** | 默认拒绝，只允许已知的 | 默认允许，只禁止已知的 |
> | **安全级别** | 🟢 高（只放行已知安全地址） | 🟡 低（可能漏掉未知风险地址） |
> | **适用场景** | Agent 只需跟特定几个协议交互（如只做 Uniswap swap） | Agent 操作范围广，但需屏蔽已知的钓鱼/恶意地址 |
> | **新增地址** | 每增加一个可交互地址需手动添加 | 只在发现新风险地址时添加 |
> | **风险** | 可能遗漏合法地址导致交易失败 | 可能遗漏新的风险地址导致资金损失 |
>
> **实际建议**：对于 AI Agent 钱包，优先使用**白名单模式**——因为 Agent 的操作范围通常是可预知的，白名单的"默认拒绝"策略更安全。
